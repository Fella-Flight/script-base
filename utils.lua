...
function unitsAliveInGroup(grp)
  if not grp or not grp.getUnits then return 0 end
  local alive_units = 0
...
randomOffset = function(absMin,absMax)
    local modifier_x = (math.random(0,1)*2) - 1
    local modifier_y = (math.random(0,1)*2) - 1
    return {
